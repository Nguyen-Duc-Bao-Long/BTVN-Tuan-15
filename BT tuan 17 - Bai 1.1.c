...
int deleteNode(Node *root, char *name){
    if (root == NULL)
        return 0;
...
int updatePages(Node *p) {
    if (p == NULL) return 0;
    //Node la la thi giu nguyen
    if (p->children == NULL) {
        return p->pages;
    }
   //Neu co con => tinh tong
    int sum = 0;
    Node *child = p->children;
    while (child != NULL) {
        sum += updatePages(child);  // cap nhat con trc roi cong don
        child = child->siblings;
    }
    p->pages = sum;  //gan lai so trang cho node hien tai
    return sum;
}
